...
export class NormalizeService {
  ...
  public normalizeFilterObj(filterObj: IFilterObj) {
    return this.dataStats$.pipe(
      map(cacheCol => {
        filterObj.filterRestaurants = this.normalizeCollection(
          'restaurants',
          filterObj.filterRestaurants,
          cacheCol
        );
        filterObj.filterItems = this.normalizeCollection(
          'items',
          filterObj.filterItems,
          cacheCol
        );
        filterObj.filterNutrients = this.normalizeCollection(
          'nutrients',
  ...
  private normalizeCollection(
    name: string,
    filters: any[],
    cacheCol: Collection<IItemStats>
  ) {
    if (filters.length <= 0) {
      return filters;
